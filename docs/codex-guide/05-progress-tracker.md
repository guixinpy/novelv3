...
- [x] Trace Anomaly Trends 只读审计意图：自然语言“检查第4章 Trace 异常趋势 limit 9 baseline 6”可投影为 inspect_agent_trace_anomaly_trends 只读工具计划，Trace Anomaly Trends run 可在 AgentRunDrawer 展示趋势、基线、阈值信号和阈值校准安全摘要
...
- [x] Dogfood Evidence 只读审计意图：自然语言“检查 dogfood pressure-test 证据覆盖”可投影为 inspect_agent_dogfood_evidence 只读工具计划，并覆盖 Trace anomaly threshold calibration 证据
...
- [x] 写作质量诊断：dogfood_evidence_projection，可由自然语言“检查 dogfood pressure-test 证据覆盖”直接规划到 inspect_agent_dogfood_evidence，并暴露 Trace anomaly threshold calibration 覆盖与真实长跑样本复核缺口
...
- [x] Trace Anomaly Trends 自然语言只读入口：自然语言“检查第 N 章 Trace 异常趋势 limit <n> baseline <n>”可直接规划到 inspect_agent_trace_anomaly_trends，后端可聚合最近 run 的受影响数量、严重度、问题类型、主要问题、baseline window、rate delta、阈值信号、阈值校准建议、误报/漏报 guard 和推荐后续，AgentRunDrawer 可展示安全摘要并隐藏 raw run/step/trace/context/calibration 内部字段
- [x] 基础上下文压缩：对话历史长度限制
...
| P4 | Trace 聚合异常检测 | 单 run 异常摘要 + 跨 run 趋势和异常统计 | 🟡 进行中（单 run anomaly_summary + 最近 run anomaly trends + baseline/threshold signals + runtime calibration + Drawer + dogfood calibration evidence 已完成；真实长跑 dogfood 样本复核与阈值固化策略待补） |
...
- [x] AgentRunDrawer Trace Anomaly Trends 投影：展示 inspect_agent_trace_anomaly_trends 的趋势状态、运行/受影响/问题/严重度计数、问题类型、baseline window、rate delta、阈值信号、阈值校准、误报/漏报 guard、受影响 run 摘要和推荐后续，同时隐藏 project/run/step/trace/context/signal/calibration 内部字段
