...
- [x] AgentRunDrawer 投影拆分约束：ADR-010 已明确 Drawer Shell + Projection Panel + Shared Projector 架构；新增复杂投影优先拆为独立小组件和专属测试，避免继续向 7000 行级 Drawer 主文件堆叠逻辑；Write Gate Coverage、Reference Alignment、Agent Event Projection、Agent Job Projection、Chapter Conflict Recovery 与 Worker Dispatch 安全投影已按该约束落地，Drawer 主文件已降到 7000 行以下
