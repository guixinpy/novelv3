...
- [x] Agent 能力纵切架构与文件规模预算：ADR-011 已明确 Capability Slice + Thin Shell + Explicit Budget；触碰 2000 行以上文件时原则上不得净增长，新增超过约 50 行能力逻辑应先拆能力文件或说明原因，当前已完成 Trace Anomaly 系列与 Memory Tree LLM candidate / batch prepare / batch execute 投影迁出，后续优先继续压缩 AgentRunDrawer、AgentRunDrawer.test 和大型 fixture
