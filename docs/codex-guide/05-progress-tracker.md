...
- [x] AgentRunDrawer 执行计划进度摘要：展示计划工具数、已执行、已完成、进行中和下一步工具
...
| 优先级 | 任务 | 完成标准 | 状态 |
|--------|------|---------|------|
| P1 | Agent 执行计划可视化 | 对话中展示当前执行计划、工具调用进度 | 🟡 进行中（Drawer plan progress + followup pending confirmation fallback） |
