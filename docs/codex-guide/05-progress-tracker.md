...
- [x] Trace Anomaly Trends 只读审计意图：自然语言“检查第4章 Trace 异常趋势 limit 9 baseline 6”可投影为 inspect_agent_trace_anomaly_trends 只读工具计划，Trace Anomaly Trends run 可在 AgentRunDrawer 展示安全摘要
...
- [x] Trace Anomaly Trends 自然语言只读入口：自然语言“检查第 N 章 Trace 异常趋势 limit <n> baseline <n>”可直接规划到 inspect_agent_trace_anomaly_trends，后端可聚合最近 run 的受影响数量、严重度、问题类型、主要问题、baseline window、rate delta、阈值信号和推荐后续，AgentRunDrawer 可展示安全摘要并隐藏 raw run/step/trace/context 内部字段
- [x] 基础上下文压缩：对话历史长度限制
...
- [x] AgentRunDrawer Trace Anomaly Trends 投影：展示 inspect_agent_trace_anomaly_trends 的趋势状态、运行/受影响/问题/严重度计数、问题类型、baseline window、rate delta、阈值信号、受影响 run 摘要和推荐后续，同时隐藏 project/run/step/trace/context/signal 内部字段
