...
- 2026-06-06: `tool_executor_static_metadata` 按 [08-实施架构框架](./08-implementation-architecture.md) 迁出 static adapter metadata、unhandled internal tool tracking 与 legacy metadata 回归到 `test_writing_agent_tool_executor_static_metadata.py`；触碰超预算文件 `backend/tests/test_writing_agent_tool_executor.py` 从 8473 行降到 7387 行，新切片 1028 行并低于 1500 行警戒线；targeted 验证 `pytest tests/test_writing_agent_tool_executor_static_metadata.py tests/test_writing_agent_tool_executor.py -q` 通过（237 passed）；剩余风险转为 executor dialog intent、tool handling 和少量 tool contract execution projection 回归仍在超预算文件中。
- 2026-06-06: `tool_executor_route_opt_in` 按 [08-实施架构框架](./08-implementation-architecture.md) 迁出 route opt-in plan/preview/contract 回归到 `test_writing_agent_tool_executor_route_opt_in.py`，并迁出 approval 回归到 `test_writing_agent_tool_executor_route_opt_in_approval.py`；触碰超预算文件 `backend/tests/test_writing_agent_tool_executor.py` 从 9400 行降到 8473 行，新切片控制在 521 / 444 行；targeted 验证 `pytest tests/test_writing_agent_tool_executor_route_opt_in.py tests/test_writing_agent_tool_executor_route_opt_in_approval.py tests/test_writing_agent_tool_executor.py -q` 通过（261 passed）；剩余风险转为 executor static metadata、dialog intent、tool handling，以及 route opt-in 的 metadata/handoff 断言仍需随对应切片继续迁出。
- 2026-06-06: `tool_executor_adapter_boundary` 按 [08-实施架构框架](./08-implementation-architecture.md) 迁出 14 个 adapter module boundary / approval-chain builder 契约测试到 `test_writing_agent_tool_executor_adapters.py`；触碰超预算文件 `backend/tests/test_writing_agent_tool_executor.py` 从 9977 行降到 9400 行，targeted 验证 `pytest tests/test_writing_agent_tool_executor_adapters.py tests/test_writing_agent_tool_executor.py -q` 通过（275 passed）；剩余风险转为 executor static metadata、route opt-in、dialog intent、tool handling 等切片仍在超预算文件中。
- 2026-06-06: `longform_batch_execution_review_route_runs` 按 [08-实施架构框架](./08-implementation-architecture.md) 迁出 9 个 `execute_longform_chapter_batch` API run 回归到 `test_writing_agent_runs_longform_batch_execution.py`，并迁出 11 个 review/route API run 回归到 `test_writing_agent_runs_longform_batch_review_route.py`；触碰超预算文件 `backend/tests/test_writing_agent_runs.py` 从 5703 行降到 4587 行，targeted 验证 `pytest tests/test_writing_agent_runs_longform_batch.py tests/test_writing_agent_runs_longform_batch_execution.py tests/test_writing_agent_runs_longform_batch_review_route.py tests/test_writing_agent_runs.py -q` 通过（138 passed）；剩余风险转为 `test_writing_agent_runs.py` 的 direct chapter recovery / 通用 API run 残留切片与 `test_writing_agent_tool_executor.py` 仍在超预算文件中。
- 2026-06-06: `longform_batch_queue_preflight_runs` 按 [08-实施架构框架](./08-implementation-architecture.md) 迁出 14 个 `plan_longform_chapter_batch` / enqueue / inspect / preflight / execution-prepare API run 回归到 `test_writing_agent_runs_longform_batch.py`，并将 longform batch enqueue / preflight / execution-review-route workflow helper 收敛到 `test_support/writing_agent_run_helpers.py`；触碰超预算文件 `backend/tests/test_writing_agent_runs.py` 从 5915 行降到 5703 行，targeted 验证 `pytest tests/test_writing_agent_runs_longform_batch.py tests/test_writing_agent_runs.py -q` 通过（138 passed）；剩余风险转为 longform batch execution/review/route 与 `test_writing_agent_tool_executor.py` 仍在超预算文件中。
- 2026-06-06: `planner_continuation_runs` 按 [08-实施架构框架](./08-implementation-architecture.md) 迁出 10 个 `ui_planner_continuation_execute` API run 回归到 `test_writing_agent_runs_planner_continuation.py`，并将 generate chapter / revision patch / world model resolution planner approval plan helper 收敛到 `test_support/writing_agent_run_helpers.py`；触碰超预算文件 `backend/tests/test_writing_agent_runs.py` 从 6572 行降到 5915 行，targeted 验证 `pytest tests/test_writing_agent_runs_planner_continuation.py tests/test_writing_agent_runs.py -q` 通过（148 passed）；剩余风险转为 longform batch 与 `test_writing_agent_tool_executor.py` 仍在超预算文件中。
- 2026-06-06: `world_model_proposal_apply_draft_runs` 按 [08-实施架构框架](./08-implementation-architecture.md) 迁出 17 个 `apply_world_model_proposal_resolution` / `draft_world_model_proposal_resolution_decisions` / high-value draft API run 回归到 `test_writing_agent_runs_world_model_proposals.py`，并将 apply approval helper 与 continuity anchor seed approval helper 收敛到 `test_support/writing_agent_run_helpers.py`；触碰超预算文件 `backend/tests/test_writing_agent_runs.py` 从 7304 行降到 6572 行，targeted 验证 `pytest tests/test_writing_agent_runs_world_model_proposals.py tests/test_writing_agent_runs.py -q` 通过（182 passed）；planner continuation 随后已由 `planner_continuation_runs` 切片迁出。
- 2026-06-06: `world_model_proposal_preview_runs` 按 [08-实施架构框架](./08-implementation-architecture.md) 迁出 8 个 `preview_world_model_proposal_resolution` / plan-preview API run 回归到 `test_writing_agent_runs_world_model_proposals.py`；触碰超预算文件 `backend/tests/test_writing_agent_runs.py` 从 8541 行降到 7304 行，targeted 验证 `pytest tests/test_writing_agent_runs_world_model_proposals.py tests/test_writing_agent_runs.py -q` 通过（182 passed）；apply/draft 世界模型提案决策链随后已由 `world_model_proposal_apply_draft_runs` 切片迁出。
- 2026-06-06: `world_model_proposal_review_plan_runs` 按 [08-实施架构框架](./08-implementation-architecture.md) 迁出 `review_world_model_proposals` 与 `plan_world_model_proposal_resolution` 的 9 个只读/阻断/串联 API run 回归到 `test_writing_agent_runs_world_model_proposals.py`，并将 `approved_generate_chapter_tool` 收敛到 `test_support/writing_agent_run_helpers.py`；触碰超预算文件 `backend/tests/test_writing_agent_runs.py` 从 8889 行降到 8541 行，targeted 验证 `pytest tests/test_writing_agent_runs_world_model_proposals.py tests/test_writing_agent_runs.py -q` 通过（182 passed）；preview/apply/draft 世界模型提案决策链随后已继续按切片迁出。
