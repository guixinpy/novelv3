...
| 机制 | 借鉴来源 | 优先级 | 说明 |
|------|---------|--------|------|
| ContextCompressor | hermes-agent | 进行中：章节 prompt block 压缩 | 已有头尾保护预修剪、摘要工具计划、dry-run payload builder、压力场景推荐入口，并在 preflight_writing 中输出压缩检查与 payload preview；章节生成 longform block 已能在压力下替换为 compressed_context，后续补持久 LLM 摘要写入和更多上下文路径分层压缩 |
...
7. **hermes-agent ContextCompressor chapter prompt block compression** → context pressure 下输出头尾保护预修剪、summarize 工具计划、只读 dry-run payload、推荐恢复入口，在 `preflight_writing` 暴露压缩检查与 payload preview，并在章节生成 longform block 中替换为 `compressed_context`
...
12. **hermes-agent ContextCompressor 持久摘要与分层压缩** → 基于章节 prompt block 压缩继续推进 LLM 摘要写入 + 更多实际上下文构建路径按重要性分层压缩
