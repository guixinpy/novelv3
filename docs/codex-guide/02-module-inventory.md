...
Agent 化缺口：
  ...
  - 前端独立 Memory Tree 面板尚未完成（当前仅运行详情 Drawer 的只读投影、自由查询、推荐 drilldown、返回节点展开和项目级会话浏览历史）
  - 与 Retrieval 的深度整合
  - LLM 摘要质量与真实长篇数据验证
...
```
当前状态：L2 对话界面含 action cards、followup、trace 入口，AgentRunDrawer 可展示计划工具/执行进度/下一步、逐项工具状态和 Memory Tree 只读节点投影，并支持自由搜索、推荐 drilldown 或返回节点展开继续发起只读浏览 run，projectWorkspace 会在同一项目内保留安全的 Memory Tree 浏览历史供 Drawer 恢复；recommended followup fallback view 可展示待确认后继并阻止 pending-only 自动执行
...
Agent 化缺口：
  ...
  - World Model / Memory 面板的整合（Memory Tree 当前仅 Drawer 只读投影、自由搜索、推荐展开、返回节点展开和项目级会话浏览历史，仍缺独立可交互面板）
