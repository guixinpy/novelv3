...
Agent 化缺口：
  ...
  - 前端独立 Memory Tree 面板已具备子导航只读搜索、历史入口、最近结果安全摘要、结果节点只读展开和当前展开状态；仍缺完整独立树视图和跨面板导航
  - 与 Retrieval 的深度整合
  - LLM 摘要质量与真实长篇数据验证
...
Agent 化缺口：
  ...
  - World Model / Memory 面板的整合（Memory Tree 当前已有 Drawer 只读投影、自由搜索、推荐展开、返回节点展开、项目级会话浏览历史和子导航搜索/历史/最近结果摘要/结果节点展开与当前展开状态，仍缺完整独立树视图和跨面板导航）
