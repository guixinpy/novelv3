...
测试切片：
  ...
  backend/tests/test_writing_agent_tool_executor_static_metadata.py     # executor static adapter metadata / unhandled tool tracking 契约回归
...
Agent 化缺口：
  ...
  - 前端独立 Memory Tree 面板已具备 Hermes Memory 主工作区、子导航只读搜索、历史入口、当前返回节点层级树、安全摘要、结果节点只读展开、当前展开状态、章节正文深链基础、Retrieval 证据只读 run 深链与 Drawer 安全摘要、Longform Context Summary 只读 run 深链与 Drawer 安全摘要、Memory Activation Plan 只读 run 深链与 Drawer 安全摘要、Memory Route 独立 Panel 安全摘要、Knowledge Base Route 独立 Panel 安全摘要、Post Chapter Memory Capture 独立 Panel 安全摘要、Trace Audit 独立 Panel 安全摘要，以及 Athena 世界模型路由深链与独立 Panel 安全摘要；仍缺更完整的独立树工作区能力
  - 与 Retrieval 的深度整合
...
```
当前状态：L1-L2 基本知识条目管理 + 候选生成；Knowledge Base Route 与写后记忆捕获计划可由自然语言只读意图触达；AgentRunDrawer 独立 Panel 可结构化展示 inspect_agent_knowledge_base_route 的状态、章节/query、作者偏好、学习规则、知识库候选、写法参考、推荐工具和诊断摘要；AgentRunDrawer 独立 Panel 可展示 plan_post_chapter_memory_capture 的章节可用性、审稿证据、候选标题/类型/摘要/置信度、来源覆盖和推荐工具安全摘要，Drawer 记忆闭环区仍可从写后记忆捕获候选的 next_tool_call 发起 prepare_record_agent_knowledge_base_candidate 只读审批准备 continuation，在待审批写入区显示清洗后的候选标题、触发 execute_record_agent_knowledge_base_candidate_with_approval 已审批执行 payload；执行成功后可展示知识库候选写入结果、候选类型、数量和推荐下一步工具，并可从 inspect_agent_knowledge_base_route 推荐发起只读检查 continuation
...
Agent 化缺口：
  ...
  - World Model / Memory 面板的整合（Memory Tree 当前已有 Drawer 只读投影、自由搜索、推荐展开、返回节点展开、Memory Tree LLM 候选摘要 Drawer 安全投影、物化状态、安全去重与多候选 prepare continuation、batch prepare Drawer 投影与逐条 execute handoff、batch execute 结果投影、项目级会话浏览历史、Hermes Memory 主工作区、子导航搜索/历史/当前返回节点层级树/结果节点展开与当前展开状态、章节正文深链基础、Retrieval 策略/策略质量/证据只读 run 深链与 Drawer 安全摘要、Longform Context Summary 只读 run 深链与 Drawer 安全摘要、Memory Activation Plan 只读 run 深链与 Drawer 安全摘要、Memory Route 独立 Panel 安全摘要、Knowledge Base Route 独立 Panel 安全摘要、Post Chapter Memory Capture 独立 Panel 安全摘要、写后记忆候选 prepare approval continuation、execute approval payload、执行成功投影与写入后 Knowledge Base Route 只读检查、Trace Audit 独立 Panel 安全摘要，以及 Athena 世界模型路由深链与独立 Panel 安全摘要、L5 语义检查独立 Panel 安全摘要、World Model Proposal Review 独立 Panel 安全摘要、World Model Proposal Resolution 独立 Panel 安全摘要；仍缺更完整独立树工作区能力）
...
| 日期 | 模块 | 变更 |
|------|------|------|
| 2026-06-06 | Backend Agent Tests | 按 08 实施架构框架将 `test_writing_agent_tool_executor.py` 中 static adapter metadata 与 unhandled tool tracking 回归迁出到 `test_writing_agent_tool_executor_static_metadata.py`；旧巨型测试文件从 8473 行降到 7387 行，新切片为 1028 行 |
...
| 2026-06-06 | Frontend Agent UX | 按 ADR-011 将 inspect_agent_trace_anomaly_long_run_samples 与 inspect_agent_trace_anomaly_threshold_review 安全投影迁出为独立 Panel，并新增专属脱敏测试；AgentRunDrawer 主文件降到 3300 行级 |
| 2026-06-06 | Frontend Agent UX | 按 ADR-011 将 inspect_agent_trace_anomaly_trends 安全投影迁出为 AgentRunTraceAnomalyTrendsPanel，并新增专属脱敏测试；AgentRunDrawer 主文件降到 3600 行级 |
| 2026-06-06 | Frontend Agent UX | 按 ADR-010 将 inspect_agent_trace_audit 安全投影迁出为 AgentRunTraceAuditPanel，并新增专属脱敏测试；AgentRunDrawer 主文件降到 4100 行级，Drawer 测试移除重复 Trace Audit 细节断言后降到 5600 行级 |
...
| 2026-06-06 | Frontend Agent UX | 按 ADR-010 将 plan_post_chapter_memory_capture 安全投影迁出为 AgentRunPostChapterMemoryPanel，并新增专属脱敏测试；AgentRunDrawer 保留记忆闭环轻量计数和候选 prepare action，主文件降到 5100 行以下 |
| 2026-06-06 | Frontend Agent UX | 按 ADR-010 将 inspect_agent_knowledge_base_route 安全投影迁出为 AgentRunKnowledgeBaseRoutePanel，并新增专属脱敏测试；AgentRunDrawer 继续收窄为输出定位和面板挂载，主文件降到 5200 行以下 |
| 2026-06-06 | Frontend Agent UX | 按 ADR-010 将 inspect_agent_memory_route 安全投影迁出为 AgentRunMemoryRoutePanel，并新增专属脱敏测试；AgentRunDrawer 继续收窄为输出定位和面板挂载 |
...
| 2026-06-06 | Frontend Agent UX | 按 ADR-010 将 inspect_agent_memory_activation_plan 安全投影迁出为 AgentRunMemoryActivationPanel，并新增专属脱敏测试；AgentRunDrawer 仅保留记忆闭环摘要所需轻量计数 |
...
| 2026-06-06 | Frontend Agent UX | 按 ADR-010 将 inspect_agent_retrieval_strategy_quality 安全投影迁出为 AgentRunRetrievalStrategyQualityPanel，并新增专属脱敏测试；AgentRunDrawer 继续收窄为输出定位和面板挂载 |
| 2026-06-06 | Frontend Agent UX | 按 ADR-010 将 inspect_agent_retrieval_strategy 安全投影迁出为 AgentRunRetrievalStrategyPanel，并新增专属脱敏测试；AgentRunDrawer 继续收窄为输出定位和面板挂载 |
| 2026-06-06 | Frontend Agent UX | 按 ADR-010 将 inspect_agent_worker_dispatch 安全投影迁出为 AgentRunWorkerDispatchPanel，并新增专属脱敏测试；AgentRunDrawer 继续收窄为输出定位和面板挂载 |
| 2026-06-06 | Frontend Agent UX | 按 ADR-010 将 inspect_agent_job_projection 与 plan_chapter_conflict_recovery 安全投影迁出为独立 Panel，并新增专属脱敏测试；AgentRunDrawer 主文件降到 7000 行以下，只保留工具输出定位和面板挂载 |
| 2026-06-06 | Frontend Agent UX | 按 ADR-010 将 inspect_agent_event_projection 安全投影从 AgentRunDrawer 主文件迁出为 AgentRunEventProjectionPanel，并新增专属脱敏测试；Drawer 集成测试收窄为面板挂载验证，主文件降到 7000 行以下 |
