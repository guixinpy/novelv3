...
- 当前 Frontend Agent UX 的优先拆分对象是 `AgentRunDrawer.vue`、`AgentRunDrawer.test.ts`、剩余 Memory Tree 候选检查 / batch prepare 投影和 Drawer fixture；Memory Tree batch execute 结果、Trace Audit 与 Trace Anomaly 系列已迁出为独立 Panel，并应继续向 fixture/projector 拆分收敛。
