...
- 当前 Backend Agent Tests 的优先拆分对象是 `test_writing_agent_runs.py` 与 `test_writing_agent_tool_executor.py`；`test_writing_agent_runs.py` 已先迁出 revision draft API 回归和共享 test_support helper，后续继续按 planner continuation、longform batch、world model proposal resolution、chapter resize 等能力切片拆分。
