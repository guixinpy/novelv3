...
- 当前 Frontend Agent UX 的优先拆分对象是 `AgentRunDrawer.vue`、`AgentRunDrawer.test.ts` 和剩余大块 Memory Tree 候选/执行投影；Trace Audit 与 Trace Anomaly 系列已迁出为独立 Panel，并应继续向 fixture/projector 拆分收敛。
