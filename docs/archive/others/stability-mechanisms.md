...
- CLI 也可以用 `--max-writing-under-target`、`--max-writing-over-target`、`--max-writing-warnings`、`--max-writing-outline-like` 做写作质量风险门禁。
- smoke 报告会输出 `writing_worker.generation_diagnostics`，用于检查本轮连续写作是否出现偏短、偏长、大纲式章节或维护警告。
- smoke 报告会输出 `maintenance.ready_for_writing` 和 `maintenance.issue_count`，CLI 默认在长篇维护未就绪时失败，避免记忆/检索未同步却继续判定为可写。
