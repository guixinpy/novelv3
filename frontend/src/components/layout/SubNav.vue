...
<template>
  <aside class="subnav" :class="{ 'subnav--collapsed': collapsed }">
    ...
    <button v-if="!collapsed" class="subnav__toggle" aria-label="收起侧栏" @click="emit('toggle-collapse')">
      ‹
    </button>
  </aside>
</template>
...
.subnav__toggle {
  ...
  border-radius: var(--radius-sm); color: var(--color-text-tertiary); font-size: var(--text-sm);
