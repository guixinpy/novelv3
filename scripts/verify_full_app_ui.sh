...
wait_for_eval_true "切到设定后出现三张概览卡" "(() => { const cards = [ ['setup-summary-card-characters', '角色'], ['setup-summary-card-world', '世界观'], ['setup-summary-card-concept', '核心概念'] ]; return cards.every(([testId, label]) => { const card = document.querySelector('[data-testid=\"' + testId + '\"]'); return Boolean(card) && (card.textContent || '').includes(label); }); })()"
assert_eval "默认右侧不直接出现完整阅读区" "(() => { const forbiddenSelectors = ['[data-testid=\"setup-detail-modal\"]', '[data-testid=\"setup-character-detail\"]', '[data-testid=\"setup-world-card\"]', '[data-testid=\"setup-concept-card\"]', '[data-testid=\"setup-section-tabs\"]']; const hits = forbiddenSelectors.filter((selector) => document.querySelector(selector)); if (hits.length > 0) { throw new Error('默认状态仍渲染旧阅读区: ' + hits.join(',')); } return true; })()"
assert_eval "设定页面文本不暴露 setup JSON key" "(() => { const text = document.body?.innerText || ''; const forbidden = ['background', 'geography', 'society', 'rules', 'atmosphere', 'theme', 'premise', 'hook', 'unique_selling_point', 'age', 'gender', 'personality', 'goals', 'character_status']; const hits = forbidden.filter((key) => text.includes(key)); if (hits.length > 0) { throw new Error('页面暴露 setup JSON key: ' + hits.join(',')); } return true; })()"

assert_eval "点击角色卡查看完整" "(() => { const button = document.querySelector('[data-testid=\"setup-summary-card-characters\"] [data-testid=\"setup-summary-open\"]'); if (!button) { throw new Error('未找到角色卡查看完整按钮'); } button.click(); return true; })()"
wait_for_eval_true "角色卡打开 modal 并定位到角色" "(() => { const modal = document.querySelector('[data-testid=\"setup-detail-modal\"]'); const panel = document.querySelector('[data-testid=\"setup-detail-panel-characters\"]'); const tab = document.querySelector('[data-testid=\"setup-detail-tab-characters\"]'); if (!modal || !panel || !tab) { return false; } if (panel.getAttribute('aria-hidden') !== 'false') { return false; } if (tab.getAttribute('aria-selected') !== 'true') { return false; } const text = panel.textContent || ''; const expected = ['沈砚', '冷静克制，但对真相近乎偏执。', '旧城档案馆的修复员。', '查清师父失踪与记忆税黑市的关联。']; return expected.every((item) => text.includes(item)); })()"
