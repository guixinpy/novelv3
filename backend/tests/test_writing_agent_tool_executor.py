...
@pytest.mark.asyncio
async def test_tool_executor_handles_dialog_intent_agent_plan_for_knowledge_base_route_read(db_session):
    project = Project(name="Dialog Intent Knowledge Base Route Plan")
    db_session.add(project)
    db_session.commit()

    result = await execute_writing_agent_tool(
        WritingAgentToolContext(db=db_session, project_id=project.id, run_id="run-dialog-intent-knowledge-base-route"),
        WritingAgentToolRequest(
            tool_name="plan_dialog_intent_agent_run",
            params={"text": "检查第4章知识库路由 query=写法偏好 limit 9"},
        ),
    )
    ...
    assert result.output is not None
    assert result.output["status"] == "completed"
    assert result.output["intent_projection"]["rule_id"] == "knowledge_base_route_intent"
    assert result.output["planner"]["intent_class"] == "inspect_knowledge_base_route"
    assert result.output["planner"]["mapped_from_action_type"] == "inspect_knowledge_base_route"
    ...
    assert result.output["plan"] == {
        ...
        "tools": [{"tool_name": "inspect_agent_knowledge_base_route", "params": expected_params}],
        "approval_contract": {"status": "not_required", "write_steps": []},
    }
    assert result.output["tools"] == [{"tool_name": "inspect_agent_knowledge_base_route", "params": expected_params}]
    assert result.output["approval_contract"] == {"status": "not_required", "write_steps": []}
    assert result.output["trace"]["reason"] == "planned_direct_read_tool_from_intent_projection"


@pytest.mark.asyncio
async def test_tool_executor_handles_dialog_intent_agent_plan_for_post_chapter_memory_capture_read(db_session):
    project = Project(name="Dialog Intent Post Chapter Memory Capture Plan")
    db_session.add(project)
    db_session.commit()
    ...
    result = await execute_writing_agent_tool(
        WritingAgentToolContext(
            db=db_session,
            project_id=project.id,
            run_id="run-dialog-intent-post-chapter-memory-capture",
        ),
        WritingAgentToolRequest(
            tool_name="plan_dialog_intent_agent_run",
            params={"text": "规划第4章写后记忆沉淀"},
        ),
    )
    ...
    assert result.output is not None
    assert result.output["status"] == "completed"
    assert result.output["intent_projection"]["rule_id"] == "post_chapter_memory_capture_intent"
    assert result.output["planner"]["intent_class"] == "plan_post_chapter_memory_capture"
    assert result.output["planner"]["mapped_from_action_type"] == "plan_post_chapter_memory_capture"
    assert result.output["planner"]["chapter_index"] == 4
    assert result.output["plan"] == {
        "status": "completed",
        "intent_class": "plan_post_chapter_memory_capture",
        "steps": [
            {
                "step_index": 1,
        ...
        "tools": [{"tool_name": "plan_post_chapter_memory_capture", "params": expected_params}],
        "approval_contract": {"status": "not_required", "write_steps": []},
    }
    assert result.output["tools"] == [{"tool_name": "plan_post_chapter_memory_capture", "params": expected_params}]
    assert result.output["approval_contract"] == {"status": "not_required", "write_steps": []}
    assert result.output["trace"]["reason"] == "planned_direct_read_tool_from_intent_projection"
...
@pytest.mark.asyncio
async def test_tool_executor_handles_dialog_intent_agent_plan_for_retrieval_strategy_read(db_session):
    project = Project(name="Dialog Intent Retrieval Strategy Plan")
    db_session.add(project)
    db_session.commit()

    result = await execute_writing_agent_tool(
        WritingAgentToolContext(db=db_session, project_id=project.id, run_id="run-dialog-intent-retrieval-strategy"),
        WritingAgentToolRequest(
            tool_name="plan_dialog_intent_agent_run",
            params={"text": "规划第5章检索策略 query=旧灯塔回声 limit 6 candidate_limit 50"},
        ),
    )
    ...
    assert result.output is not None
    assert result.output["status"] == "completed"
    assert result.output["intent_projection"]["rule_id"] == "retrieval_strategy_intent"
    assert result.output["planner"]["intent_class"] == "inspect_retrieval_strategy"
    assert result.output["planner"]["mapped_from_action_type"] == "inspect_retrieval_strategy"
    assert result.output["planner"]["chapter_index"] == 5
    assert result.output["plan"] == {
        "status": "completed",
        "intent_class": "inspect_retrieval_strategy",
        "steps": [
            {
                "step_index": 1,
        ...
        "tools": [{"tool_name": "inspect_agent_retrieval_strategy", "params": expected_params}],
        "approval_contract": {"status": "not_required", "write_steps": []},
    }
    assert result.output["tools"] == [{"tool_name": "inspect_agent_retrieval_strategy", "params": expected_params}]
    assert result.output["approval_contract"] == {"status": "not_required", "write_steps": []}
    assert result.output["trace"]["reason"] == "planned_direct_read_tool_from_intent_projection"
...
@pytest.mark.asyncio
async def test_tool_executor_handles_dialog_intent_agent_plan_for_preflight_context_budget_read(db_session):
    project = Project(name="Dialog Intent Preflight Context Budget Plan")
    db_session.add(project)
    db_session.commit()
    ...
    result = await execute_writing_agent_tool(
        WritingAgentToolContext(
            db=db_session,
            project_id=project.id,
            run_id="run-dialog-intent-preflight-context-budget-plan",
        ),
        WritingAgentToolRequest(
            tool_name="plan_dialog_intent_agent_run",
            params={
                "text": "预检第3章上下文预算 max_context_chars 500 context_guard_failure_count 2"
            },
        ),
    )
    ...
    assert result.output is not None
    assert result.output["status"] == "completed"
    assert result.output["intent_projection"]["rule_id"] == "preflight_context_budget_intent"
    assert result.output["planner"]["intent_class"] == "preflight_context_budget"
    assert result.output["planner"]["mapped_from_action_type"] == "preflight_context_budget"
    assert result.output["planner"]["chapter_index"] == 3
    assert result.output["plan"] == {
        "status": "completed",
        ...
        "tools": [{"tool_name": "preflight_writing", "params": expected_params}],
        "approval_contract": {"status": "not_required", "write_steps": []},
    }
    assert result.output["tools"] == [{"tool_name": "preflight_writing", "params": expected_params}]
    assert result.output["approval_contract"] == {"status": "not_required", "write_steps": []}
    assert result.output["trace"]["reason"] == "planned_direct_read_tool_from_intent_projection"
...
@pytest.mark.asyncio
async def test_tool_executor_handles_dialog_intent_agent_plan_for_agent_job_projection_read(db_session):
    project = Project(name="Dialog Intent Agent Job Projection Plan")
    db_session.add(project)
    db_session.commit()
    ...
    result = await execute_writing_agent_tool(
        WritingAgentToolContext(
            db=db_session,
            project_id=project.id,
            run_id="run-dialog-intent-agent-job-projection-plan",
        ),
        WritingAgentToolRequest(
            tool_name="plan_dialog_intent_agent_run",
            params={"text": "检查第3章 generate_chapter failed 任务队列 limit 8"},
        ),
    )
    ...
    assert result.output is not None
    assert result.output["status"] == "completed"
    assert result.output["intent_projection"]["rule_id"] == "agent_job_projection_intent"
    assert result.output["planner"]["intent_class"] == "inspect_agent_job_projection"
    assert result.output["planner"]["mapped_from_action_type"] == "inspect_agent_job_projection"
    assert result.output["planner"]["chapter_index"] == 3
    assert result.output["plan"] == {
        "status": "completed",
        "intent_class": "inspect_agent_job_projection",
        "steps": [
            {
                "step_index": 1,
        ...
        "tools": [{"tool_name": "inspect_agent_job_projection", "params": expected_params}],
        "approval_contract": {"status": "not_required", "write_steps": []},
    }
    assert result.output["tools"] == [{"tool_name": "inspect_agent_job_projection", "params": expected_params}]
    assert result.output["approval_contract"] == {"status": "not_required", "write_steps": []}
    assert result.output["trace"]["reason"] == "planned_direct_read_tool_from_intent_projection"
...
@pytest.mark.asyncio
async def test_tool_executor_handles_dialog_intent_agent_plan_for_trace_anomaly_long_run_samples_read(db_session):
    project = Project(name="Dialog Intent Trace Anomaly Long Run Samples Plan")
    db_session.add(project)
    db_session.commit()
    ...
    result = await execute_writing_agent_tool(
        WritingAgentToolContext(
            db=db_session,
            project_id=project.id,
            run_id="run-dialog-intent-trace-long-run-samples-plan",
        ),
        WritingAgentToolRequest(
            tool_name="plan_dialog_intent_agent_run",
            params={"text": "检查第4章 Trace 异常长跑样本 limit 9"},
        ),
    )
    ...
    assert result.output is not None
    assert result.output["status"] == "completed"
    assert result.output["intent_projection"]["rule_id"] == "trace_anomaly_long_run_samples_intent"
    assert result.output["planner"]["intent_class"] == "inspect_trace_anomaly_long_run_samples"
    assert result.output["planner"]["mapped_from_action_type"] == "inspect_trace_anomaly_long_run_samples"
    assert result.output["planner"]["chapter_index"] == 4
    assert result.output["plan"] == {
        "status": "completed",
        "intent_class": "inspect_trace_anomaly_long_run_samples",
        "steps": [
            {
                "step_index": 1,
        ...
        "tools": [{"tool_name": "inspect_agent_trace_anomaly_long_run_samples", "params": expected_params}],
        "approval_contract": {"status": "not_required", "write_steps": []},
    }
    assert result.output["tools"] == [
        {"tool_name": "inspect_agent_trace_anomaly_long_run_samples", "params": expected_params}
    ]
    assert result.output["approval_contract"] == {"status": "not_required", "write_steps": []}
    assert result.output["trace"]["reason"] == "planned_direct_read_tool_from_intent_projection"
...
async def test_tool_executor_handles_dialog_intent_agent_plan_for_agent_event_projection_read(db_session):
    ...
    result = await execute_writing_agent_tool(
        WritingAgentToolContext(
            db=db_session,
            project_id=project.id,
            run_id="run-dialog-intent-agent-event-projection-plan",
        ),
        WritingAgentToolRequest(
            tool_name="plan_dialog_intent_agent_run",
            params={"text": "检查 task-abc123 的 Agent 事件投影 limit 12"},
        ),
    )
    ...
    assert result.output is not None
    assert result.output["status"] == "completed"
    assert result.output["intent_projection"]["rule_id"] == "agent_event_projection_intent"
    assert result.output["planner"]["intent_class"] == "inspect_agent_event_projection"
    assert result.output["planner"]["mapped_from_action_type"] == "inspect_agent_event_projection"
    assert result.output["planner"]["chapter_index"] is None
    assert result.output["plan"] == {
        "status": "completed",
        "intent_class": "inspect_agent_event_projection",
        "steps": [
            {
                "step_index": 1,
        ...
        "tools": [{"tool_name": "inspect_agent_event_projection", "params": expected_params}],
        "approval_contract": {"status": "not_required", "write_steps": []},
    }
    assert result.output["tools"] == [{"tool_name": "inspect_agent_event_projection", "params": expected_params}]
    assert result.output["approval_contract"] == {"status": "not_required", "write_steps": []}
    assert result.output["trace"]["reason"] == "planned_direct_read_tool_from_intent_projection"


@pytest.mark.asyncio
async def test_tool_executor_handles_dialog_intent_agent_plan_for_chapter_conflict_recovery_read(db_session):
    project = Project(name="Dialog Intent Chapter Conflict Recovery Plan")
    db_session.add(project)
    db_session.commit()
    ...
    result = await execute_writing_agent_tool(
        WritingAgentToolContext(
            db=db_session,
            project_id=project.id,
            run_id="run-dialog-intent-chapter-conflict-recovery-plan",
        ),
        WritingAgentToolRequest(
            tool_name="plan_dialog_intent_agent_run",
            params={"text": "规划第3章章节冲突恢复"},
        ),
    )
    ...
    assert result.output is not None
    assert result.output["status"] == "completed"
    assert result.output["intent_projection"]["rule_id"] == "chapter_conflict_recovery_intent"
    assert result.output["planner"]["intent_class"] == "plan_chapter_conflict_recovery"
    assert result.output["planner"]["mapped_from_action_type"] == "plan_chapter_conflict_recovery"
    assert result.output["planner"]["chapter_index"] == 3
    assert result.output["plan"] == {
        "status": "completed",
        "intent_class": "plan_chapter_conflict_recovery",
        "steps": [
            {
                "step_index": 1,
        ...
        "tools": [{"tool_name": "plan_chapter_conflict_recovery", "params": expected_params}],
        "approval_contract": {"status": "not_required", "write_steps": []},
    }
    assert result.output["tools"] == [{"tool_name": "plan_chapter_conflict_recovery", "params": expected_params}]
    assert result.output["approval_contract"] == {"status": "not_required", "write_steps": []}
    assert result.output["trace"]["reason"] == "planned_direct_read_tool_from_intent_projection"
...
@pytest.mark.asyncio
async def test_tool_executor_dispatches_inspect_agent_trace_anomaly_long_run_samples_adapter(
    db_session,
    monkeypatch,
):
    project = Project(name="Executor Trace Anomaly Long Run Samples")
    db_session.add(project)
    db_session.commit()
    calls: list[tuple[str, int | None, int | None, int | None]] = []
    ...
    def fake_samples(
        ...
        *,
        limit: int | None,
        chapter_index: int | None,
        minimum_review_run_count: int | None,
    ):
        calls.append((project_id, limit, chapter_index, minimum_review_run_count))
        return {"status": "completed", "sample_collection": {"status": "ready_for_threshold_review"}}

    monkeypatch.setattr(
        "app.services.writing_agent.agent_trace_audit.inspect_agent_trace_anomaly_long_run_samples",
        fake_samples,
    )
    ...
    result = await execute_writing_agent_tool(
        ...
        WritingAgentToolRequest(
            tool_name="inspect_agent_trace_anomaly_long_run_samples",
            params={"limit": "9", "chapter_index": "4", "minimum_review_run_count": "6"},
        ),
    )

    assert result.handled is True
    assert result.output == {"status": "completed", "sample_collection": {"status": "ready_for_threshold_review"}}
    assert calls == [(project.id, 9, 4, 6)]
