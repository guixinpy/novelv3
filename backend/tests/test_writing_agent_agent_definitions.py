...
def test_worker_dispatch_routes_memory_tree_summary_materialization_to_memory_worker():
    preview = preview_agent_worker_dispatches(
        [
            ...
            },
            {
                ...
                "params": {"candidate_trace_ids": ["trace-1", "trace-2"]},
            },
            {
            ...
            },
            {
                ...
                "params": {"confirm_execute": True, "candidate_executions": []},
            },
            {
            ...
            },
            {
                ...
                "params": {"quality_chapter_index": 2},
            },
            {
    ...
    assert preview["status"] == "ready"
    assert preview["summary"] == {"workers": 1, "planned_tasks": 9, "blocked_tasks": 0, "issues": 0}
    dispatch = preview["worker_dispatches"][0]
    assert dispatch["worker"]["name"] == "memory_worker"
    assert [item["tool_name"] for item in dispatch["task_envelopes"]] == [
...
def test_worker_route_registry_audit_binds_routes_to_allowed_worker_definitions():
    ...
    assert audit["status"] == "passed"
    assert audit["summary"] == {"routes": 72, "ready_routes": 72, "unrouted_allowed_tools": 0, "issues": 0}
    assert audit["issues"] == []
    ...
    assert (
        routes_by_tool["execute_record_agent_memory_tree_llm_candidate_summaries_batch_with_approval"]["worker"]
        == "memory_worker"
    )
    ...
    assert routes_by_tool["prepare_record_agent_memory_tree_summaries"]["worker"] == "memory_worker"
    ...
    assert routes_by_tool["inspect_agent_retrieval_prefetch_plan"]["worker"] == "retrieval_worker"
