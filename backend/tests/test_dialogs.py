...
def test_intent_router_projection_explains_memory_tree_llm_summary_plan_route():
    router = IntentRouter()
    diag = ProjectDiagnosisOut(
        missing_items=[],
        completed_items=["setup", "storyline", "outline", "content"],
    ...
    projection = router.project("构建第2章记忆树 LLM 摘要计划 query=灯塔旧回声 max_chars 180", "chatting", None, diag).to_dict()

    assert projection["status"] == "matched"
    assert projection["rule_id"] == "memory_tree_llm_summary_plan_intent"
    assert projection["decision"]["rule_id"] == "memory_tree_llm_summary_plan_intent"
    assert projection["decision"]["match_evidence"] == [
        {"kind": "pattern", "name": "memory_tree_llm_summary_plan_phrase"}
    ]
    assert projection["candidate"] == {
        "type": "build_memory_tree_llm_summary_plan",
    ...
    assert projection["tool_selection"] == {
        "selected_tool": "build_agent_memory_tree_llm_summary_plan",
        "why_this_tool": "dialog_action_to_agent_tool.build_memory_tree_llm_summary_plan",
        "availability_checked": False,
    }
    assert projection["extracted_params"] == {"chapter_index": 2, "query": "灯塔旧回声", "max_source_chars": 180}


def test_intent_router_projection_explains_memory_tree_llm_summary_candidate_route():
    router = IntentRouter()
    diag = ProjectDiagnosisOut(
        missing_items=[],
        completed_items=["setup", "storyline", "outline", "content"],
    ...
    assert projection["status"] == "matched"
    assert projection["rule_id"] == "memory_tree_llm_summary_candidate_intent"
    assert projection["decision"]["rule_id"] == "memory_tree_llm_summary_candidate_intent"
    assert projection["decision"]["match_evidence"] == [
        {"kind": "pattern", "name": "memory_tree_llm_summary_candidate_phrase"}
    ]
    assert projection["candidate"] == {
        "type": "summarize_memory_tree_llm_candidate",
        "params": {"chapter_index": 2, "query": "灯塔旧回声", "max_source_chars": 180},
    }
    assert projection["agent_route"] == _expected_agent_route(
        "text_intent",
    ...
    assert projection["tool_selection"] == {
        "selected_tool": "summarize_agent_memory_tree_llm_candidate",
        "why_this_tool": "dialog_action_to_agent_tool.summarize_memory_tree_llm_candidate",
        "availability_checked": False,
    }
    assert projection["extracted_params"] == {"chapter_index": 2, "query": "灯塔旧回声", "max_source_chars": 180}
...
def test_intent_router_projection_explains_knowledge_base_route():
    router = IntentRouter()
    diag = ProjectDiagnosisOut(
        missing_items=[],
        completed_items=["setup", "storyline", "outline", "content"],
    ...
    projection = router.project("检查第4章知识库路由 query=写法偏好 limit 9", "chatting", None, diag).to_dict()

    expected_params = {"chapter_index": 4, "query": "写法偏好", "limit": 9}
    assert projection["status"] == "matched"
    assert projection["rule_id"] == "knowledge_base_route_intent"
    assert projection["decision"]["rule_id"] == "knowledge_base_route_intent"
    assert projection["decision"]["match_evidence"] == [{"kind": "pattern", "name": "knowledge_base_route_phrase"}]
    assert projection["candidate"] == {
        "type": "inspect_knowledge_base_route",
        "params": expected_params,
    }
    assert projection["agent_route"] == _expected_agent_route(
        "text_intent",
        "inspect_knowledge_base_route",
        "inspect_agent_knowledge_base_route",
        requires_confirmation=False,
    )
    assert projection["tool_selection"] == {
        "selected_tool": "inspect_agent_knowledge_base_route",
        "why_this_tool": "dialog_action_to_agent_tool.inspect_knowledge_base_route",
        "availability_checked": False,
    }
    assert projection["extracted_params"] == expected_params
...
def test_intent_router_projection_explains_world_model_route():
    ...
    projection = router.project("检查第2章 char.hero 世界模型路由 limit 7", "chatting", None, diag).to_dict()

    expected_params = {"chapter_index": 2, "subject_ref": "char.hero", "limit": 7}
    assert projection["status"] == "matched"
    assert projection["rule_id"] == "world_model_route_intent"
    assert projection["decision"]["rule_id"] == "world_model_route_intent"
    assert projection["decision"]["match_evidence"] == [{"kind": "pattern", "name": "world_model_route_phrase"}]
    assert projection["candidate"] == {
        "type": "inspect_world_model_route",
        "params": expected_params,
    }
    assert projection["agent_route"] == _expected_agent_route(
        "text_intent",
        "inspect_world_model_route",
        "inspect_agent_world_model_route",
        requires_confirmation=False,
    )
    assert projection["tool_selection"] == {
        "selected_tool": "inspect_agent_world_model_route",
        "why_this_tool": "dialog_action_to_agent_tool.inspect_world_model_route",
        "availability_checked": False,
    }
    assert projection["extracted_params"] == expected_params


def test_intent_router_projection_explains_world_model_semantic_check_route():
    router = IntentRouter()
    diag = ProjectDiagnosisOut(
        missing_items=[],
        completed_items=["setup", "storyline", "outline", "content"],
    ...
    projection = router.project("语义检查第3章世界模型 subject=char.hero max_facts 6", "chatting", None, diag).to_dict()

    expected_params = {"chapter_index": 3, "subject_ref": "char.hero", "max_facts": 6}
    assert projection["status"] == "matched"
    assert projection["rule_id"] == "world_model_semantic_check_intent"
    assert projection["decision"]["rule_id"] == "world_model_semantic_check_intent"
    assert projection["decision"]["match_evidence"] == [
        {"kind": "pattern", "name": "world_model_semantic_check_phrase"}
    ]
    assert projection["candidate"] == {
        "type": "inspect_world_model_semantic_check",
        "params": expected_params,
    }
    assert projection["agent_route"] == _expected_agent_route(
        "text_intent",
        "inspect_world_model_semantic_check",
        "inspect_agent_world_model_semantic_check",
        requires_confirmation=False,
    )
    assert projection["tool_selection"] == {
        "selected_tool": "inspect_agent_world_model_semantic_check",
        "why_this_tool": "dialog_action_to_agent_tool.inspect_world_model_semantic_check",
        "availability_checked": False,
    }
    assert projection["extracted_params"] == expected_params
...
def test_intent_router_projection_explains_retrieval_strategy_route():
    router = IntentRouter()
    diag = ProjectDiagnosisOut(
        missing_items=[],
        completed_items=["setup", "storyline", "outline", "content"],
    ...
    projection = router.project("规划第5章检索策略 query=旧灯塔回声 limit 6 candidate_limit 50", "chatting", None, diag).to_dict()

    expected_params = {"chapter_index": 5, "query": "旧灯塔回声", "limit": 6, "candidate_limit": 50}
    assert projection["status"] == "matched"
    assert projection["rule_id"] == "retrieval_strategy_intent"
    assert projection["decision"]["rule_id"] == "retrieval_strategy_intent"
    assert projection["decision"]["match_evidence"] == [{"kind": "pattern", "name": "retrieval_strategy_phrase"}]
    assert projection["candidate"] == {
        "type": "inspect_retrieval_strategy",
        "params": expected_params,
    }
    assert projection["agent_route"] == _expected_agent_route(
        "text_intent",
        "inspect_retrieval_strategy",
        "inspect_agent_retrieval_strategy",
        requires_confirmation=False,
    )
    assert projection["tool_selection"] == {
        "selected_tool": "inspect_agent_retrieval_strategy",
        "why_this_tool": "dialog_action_to_agent_tool.inspect_retrieval_strategy",
        "availability_checked": False,
    }
    assert projection["extracted_params"] == expected_params


def test_intent_router_projection_explains_retrieval_strategy_quality_route():
    router = IntentRouter()
    diag = ProjectDiagnosisOut(
        missing_items=[],
        completed_items=["setup", "storyline", "outline", "content"],
    ...
    projection = router.project(
        "复核第5章检索策略质量 query=旧灯塔回声 limit 6 candidate_limit 50",
        "chatting",
        None,
        diag,
    ).to_dict()
    ...
    expected_params = {"chapter_index": 5, "query": "旧灯塔回声", "limit": 6, "candidate_limit": 50}
    assert projection["status"] == "matched"
    assert projection["rule_id"] == "retrieval_strategy_quality_intent"
    assert projection["decision"]["rule_id"] == "retrieval_strategy_quality_intent"
    assert projection["decision"]["match_evidence"] == [
        {"kind": "pattern", "name": "retrieval_strategy_quality_phrase"}
    ]
    assert projection["candidate"] == {
        "type": "inspect_retrieval_strategy_quality",
        "params": expected_params,
    }
    assert projection["agent_route"] == _expected_agent_route(
        "text_intent",
        "inspect_retrieval_strategy_quality",
        "inspect_agent_retrieval_strategy_quality",
        requires_confirmation=False,
    )
    assert projection["tool_selection"] == {
        "selected_tool": "inspect_agent_retrieval_strategy_quality",
        "why_this_tool": "dialog_action_to_agent_tool.inspect_retrieval_strategy_quality",
        "availability_checked": False,
    }
    assert projection["extracted_params"] == expected_params
...
def test_intent_router_projection_explains_context_compression_route():
    ...
    projection = router.project("检查第3章上下文压缩压力", "chatting", None, diag).to_dict()

    assert projection["status"] == "matched"
    assert projection["rule_id"] == "context_compression_intent"
    assert projection["decision"]["rule_id"] == "context_compression_intent"
    assert projection["decision"]["match_evidence"] == [{"kind": "pattern", "name": "context_compression_phrase"}]
    assert projection["candidate"] == {
        "type": "inspect_context_compression",
        "params": {"chapter_index": 3},
    }
    assert projection["agent_route"] == _expected_agent_route(
        "text_intent",
        "inspect_context_compression",
        "inspect_agent_context_compression_projection",
        requires_confirmation=False,
    )
    assert projection["tool_selection"] == {
        "selected_tool": "inspect_agent_context_compression_projection",
        "why_this_tool": "dialog_action_to_agent_tool.inspect_context_compression",
        "availability_checked": False,
    }
...
def test_intent_router_context_guard_phrase_is_case_insensitive():
    ...
    projection = router.project("检查第3章 ContextGuard 压力", "chatting", None, diag).to_dict()
...
def test_intent_router_projection_explains_agent_job_projection_route():
    router = IntentRouter()
    diag = ProjectDiagnosisOut(
        missing_items=[],
        completed_items=["setup", "storyline", "outline", "content"],
    ...
    projection = router.project("检查第3章 generate_chapter failed 任务队列 limit 8", "chatting", None, diag).to_dict()

    expected_params = {"chapter_index": 3, "task_type": "generate_chapter", "status": "failed", "limit": 8}
    assert projection["status"] == "matched"
    assert projection["rule_id"] == "agent_job_projection_intent"
    assert projection["decision"]["rule_id"] == "agent_job_projection_intent"
    assert projection["decision"]["match_evidence"] == [{"kind": "pattern", "name": "agent_job_projection_phrase"}]
    assert projection["candidate"] == {
    ...
    assert projection["tool_selection"] == {
        "selected_tool": "inspect_agent_job_projection",
        "why_this_tool": "dialog_action_to_agent_tool.inspect_agent_job_projection",
        "availability_checked": False,
    }
    assert projection["extracted_params"] == expected_params
...
def test_intent_router_projection_explains_chapter_conflict_recovery_route():
    ...
    projection = router.project("规划第3章章节冲突恢复", "chatting", None, diag).to_dict()

    expected_params = {"chapter_index": 3}
    assert projection["status"] == "matched"
    assert projection["rule_id"] == "chapter_conflict_recovery_intent"
    assert projection["decision"]["rule_id"] == "chapter_conflict_recovery_intent"
    assert projection["decision"]["match_evidence"] == [{"kind": "pattern", "name": "chapter_conflict_recovery_phrase"}]
    assert projection["candidate"] == {
    ...
    assert projection["tool_selection"] == {
        "selected_tool": "plan_chapter_conflict_recovery",
        "why_this_tool": "dialog_action_to_agent_tool.plan_chapter_conflict_recovery",
        "availability_checked": False,
    }
    assert projection["extracted_params"] == expected_params
...
def test_intent_router_projection_explains_trace_anomaly_trends_route():
    router = IntentRouter()
    diag = ProjectDiagnosisOut(
        missing_items=[],
        completed_items=["setup", "storyline", "outline", "content"],
    ...
    assert projection["decision"]["match_evidence"] == [{"kind": "pattern", "name": "trace_anomaly_trends_phrase"}]
    assert projection["candidate"] == {
        "type": "inspect_trace_anomaly_trends",
        "params": expected_params,
    }
    assert projection["agent_route"] == _expected_agent_route(
        "text_intent",
        "inspect_trace_anomaly_trends",
        "inspect_agent_trace_anomaly_trends",
        requires_confirmation=False,
    )
    assert projection["tool_selection"] == {
        "selected_tool": "inspect_agent_trace_anomaly_trends",
        "why_this_tool": "dialog_action_to_agent_tool.inspect_trace_anomaly_trends",
        "availability_checked": False,
    }
    assert projection["extracted_params"] == expected_params


def test_intent_router_projection_explains_trace_anomaly_long_run_samples_route():
    router = IntentRouter()
    diag = ProjectDiagnosisOut(
        missing_items=[],
        completed_items=["setup", "storyline", "outline", "content"],
    ...
    projection = router.project("检查第4章 Trace 异常长跑样本 limit 9", "chatting", None, diag).to_dict()

    expected_params = {"chapter_index": 4, "limit": 9}
    assert projection["status"] == "matched"
    assert projection["rule_id"] == "trace_anomaly_long_run_samples_intent"
    assert projection["decision"]["rule_id"] == "trace_anomaly_long_run_samples_intent"
    assert projection["decision"]["match_evidence"] == [
        {"kind": "pattern", "name": "trace_anomaly_long_run_samples_phrase"}
    ]
    assert projection["candidate"] == {
        "type": "inspect_trace_anomaly_long_run_samples",
        "params": expected_params,
    }
    assert projection["agent_route"] == _expected_agent_route(
        "text_intent",
        "inspect_trace_anomaly_long_run_samples",
        "inspect_agent_trace_anomaly_long_run_samples",
        requires_confirmation=False,
    )
    assert projection["tool_selection"] == {
        "selected_tool": "inspect_agent_trace_anomaly_long_run_samples",
        "why_this_tool": "dialog_action_to_agent_tool.inspect_trace_anomaly_long_run_samples",
        "availability_checked": False,
    }
    assert projection["extracted_params"] == expected_params


def test_intent_router_projection_explains_trace_anomaly_threshold_review_route():
    router = IntentRouter()
    diag = ProjectDiagnosisOut(
        missing_items=[],
        completed_items=["setup", "storyline", "outline", "content"],
    ...
    projection = router.project("复核第4章 Trace 异常阈值样本 limit 9 baseline 6", "chatting", None, diag).to_dict()

    expected_params = {"chapter_index": 4, "limit": 9, "baseline_limit": 6}
    assert projection["status"] == "matched"
    assert projection["rule_id"] == "trace_anomaly_threshold_review_intent"
    assert projection["decision"]["rule_id"] == "trace_anomaly_threshold_review_intent"
    assert projection["decision"]["match_evidence"] == [
        {"kind": "pattern", "name": "trace_anomaly_threshold_review_phrase"}
    ]
    assert projection["candidate"] == {
        "type": "inspect_trace_anomaly_threshold_review",
        "params": expected_params,
    }
    assert projection["agent_route"] == _expected_agent_route(
        "text_intent",
        "inspect_trace_anomaly_threshold_review",
        "inspect_agent_trace_anomaly_threshold_review",
        requires_confirmation=False,
    )
    assert projection["tool_selection"] == {
        "selected_tool": "inspect_agent_trace_anomaly_threshold_review",
        "why_this_tool": "dialog_action_to_agent_tool.inspect_trace_anomaly_threshold_review",
        "availability_checked": False,
    }
    assert projection["extracted_params"] == expected_params
...
def test_intent_router_projection_explains_route_approval_opt_in_apply_prepare_route():
    router = IntentRouter()
    diag = ProjectDiagnosisOut(
        missing_items=[],
        completed_items=["setup", "storyline", "outline", "content"],
    ...
    projection = router.project(
        "准备 pending-action-123 的 Agent 审批链 opt-in 执行审批",
        "chatting",
        None,
        diag,
    ).to_dict()
    ...
    expected_params = {"pending_action_id": "pending-action-123"}
    assert projection["status"] == "matched"
    assert projection["rule_id"] == "route_approval_opt_in_apply_prepare_intent"
    assert projection["decision"]["rule_id"] == "route_approval_opt_in_apply_prepare_intent"
    assert projection["decision"]["match_evidence"] == [
        {"kind": "pattern", "name": "route_approval_opt_in_apply_prepare_phrase"}
    ]
    assert projection["candidate"] == {
        "type": "prepare_route_approval_opt_in_apply",
        "params": expected_params,
    }
    assert projection["agent_route"] == _expected_agent_route(
        "text_intent",
        "prepare_route_approval_opt_in_apply",
        "prepare_apply_pending_action_route_approval_opt_in",
        requires_confirmation=False,
    )
    assert projection["tool_selection"] == {
        "selected_tool": "prepare_apply_pending_action_route_approval_opt_in",
        "why_this_tool": "dialog_action_to_agent_tool.prepare_route_approval_opt_in_apply",
        "availability_checked": False,
    }
    assert projection["extracted_params"] == expected_params
...
def test_intent_router_projection_explains_tool_contracts_route():
    ...
    projection = router.project("检查工具契约覆盖率和迁移差距", "chatting", None, diag).to_dict()

    assert projection["status"] == "matched"
    assert projection["rule_id"] == "tool_contracts_intent"
    assert projection["decision"]["rule_id"] == "tool_contracts_intent"
    assert projection["decision"]["match_evidence"] == [{"kind": "pattern", "name": "tool_contracts_phrase"}]
    assert projection["candidate"] == {
        "type": "inspect_tool_contracts",
        "params": {},
    }
    assert projection["agent_route"] == _expected_agent_route(
        "text_intent",
        "inspect_tool_contracts",
        "inspect_agent_tool_contracts",
        requires_confirmation=False,
    )
    assert projection["tool_selection"] == {
        "selected_tool": "inspect_agent_tool_contracts",
        "why_this_tool": "dialog_action_to_agent_tool.inspect_tool_contracts",
        "availability_checked": False,
    }
    assert projection["extracted_params"] == {}


def test_intent_router_projection_explains_command_contracts_route():
    router = IntentRouter()
    diag = ProjectDiagnosisOut(
        missing_items=[],
        completed_items=["setup", "storyline", "outline", "content"],
    ...
    projection = router.project("检查命令契约缺口和 slash command 投影", "chatting", None, diag).to_dict()

    assert projection["status"] == "matched"
    assert projection["rule_id"] == "command_contracts_intent"
    assert projection["decision"]["rule_id"] == "command_contracts_intent"
    assert projection["decision"]["match_evidence"] == [{"kind": "pattern", "name": "command_contracts_phrase"}]
    assert projection["candidate"] == {
        "type": "inspect_command_contracts",
        "params": {},
    }
    assert projection["agent_route"] == _expected_agent_route(
        "text_intent",
        "inspect_command_contracts",
        "inspect_agent_command_contracts",
        requires_confirmation=False,
    )
    assert projection["tool_selection"] == {
        "selected_tool": "inspect_agent_command_contracts",
        "why_this_tool": "dialog_action_to_agent_tool.inspect_command_contracts",
        "availability_checked": False,
    }
    assert projection["extracted_params"] == {}


def test_intent_router_projection_explains_slash_command_route():
    router = IntentRouter()
    diag = ProjectDiagnosisOut(
        missing_items=[],
        completed_items=["setup", "storyline", "outline", "content"],
    ...
    projection = router.project("检查 /continue 斜杠命令路由", "chatting", None, diag).to_dict()

    assert projection["status"] == "matched"
    assert projection["rule_id"] == "slash_command_route_intent"
    assert projection["decision"]["rule_id"] == "slash_command_route_intent"
    assert projection["decision"]["match_evidence"] == [{"kind": "pattern", "name": "slash_command_route_phrase"}]
    assert projection["candidate"] == {
        "type": "inspect_slash_command_route",
        "params": {"command_name": "continue"},
    }
    assert projection["agent_route"] == _expected_agent_route(
        "text_intent",
        "inspect_slash_command_route",
        "inspect_agent_slash_command_route",
        requires_confirmation=False,
    )
    assert projection["tool_selection"] == {
        "selected_tool": "inspect_agent_slash_command_route",
        "why_this_tool": "dialog_action_to_agent_tool.inspect_slash_command_route",
        "availability_checked": False,
    }
    assert projection["extracted_params"] == {"command_name": "continue"}


def test_intent_router_projection_explains_reference_alignment_route():
    router = IntentRouter()
    diag = ProjectDiagnosisOut(
        missing_items=[],
        completed_items=["setup", "storyline", "outline", "content"],
    ...
    projection = router.project("检查参考项目模式对齐和开源项目适配", "chatting", None, diag).to_dict()

    assert projection["status"] == "matched"
    assert projection["rule_id"] == "reference_alignment_intent"
    assert projection["decision"]["rule_id"] == "reference_alignment_intent"
    assert projection["decision"]["match_evidence"] == [{"kind": "pattern", "name": "reference_alignment_phrase"}]
    assert projection["candidate"] == {
        "type": "inspect_reference_alignment",
        "params": {},
    }
    assert projection["agent_route"] == _expected_agent_route(
        "text_intent",
    ...
    assert projection["tool_selection"] == {
        "selected_tool": "inspect_agent_reference_alignment",
        "why_this_tool": "dialog_action_to_agent_tool.inspect_reference_alignment",
        "availability_checked": False,
    }
    assert projection["extracted_params"] == {}


def test_intent_router_projection_explains_dogfood_evidence_route():
    router = IntentRouter()
    diag = ProjectDiagnosisOut(
        missing_items=[],
        completed_items=["setup", "storyline", "outline", "content"],
    ...
    projection = router.project("检查 dogfood pressure-test 证据覆盖", "chatting", None, diag).to_dict()

    assert projection["status"] == "matched"
    assert projection["rule_id"] == "dogfood_evidence_intent"
    assert projection["decision"]["rule_id"] == "dogfood_evidence_intent"
    assert projection["decision"]["match_evidence"] == [{"kind": "pattern", "name": "dogfood_evidence_phrase"}]
    assert projection["candidate"] == {
        "type": "inspect_dogfood_evidence",
        "params": {},
    }
    assert projection["agent_route"] == _expected_agent_route(
        "text_intent",
        "inspect_dogfood_evidence",
        "inspect_agent_dogfood_evidence",
        requires_confirmation=False,
    )
    assert projection["tool_selection"] == {
        "selected_tool": "inspect_agent_dogfood_evidence",
        "why_this_tool": "dialog_action_to_agent_tool.inspect_dogfood_evidence",
        "availability_checked": False,
    }
    assert projection["extracted_params"] == {}


def test_intent_router_projection_explains_route_preference_route():
    router = IntentRouter()
    diag = ProjectDiagnosisOut(
        missing_items=[],
        completed_items=["setup", "storyline", "outline", "content"],
    ...
    projection = router.project("检查 text_intent 路由偏好和 Agent 审批链迁移建议", "chatting", None, diag).to_dict()

    assert projection["status"] == "matched"
    assert projection["rule_id"] == "route_preference_intent"
    assert projection["decision"]["rule_id"] == "route_preference_intent"
    assert projection["decision"]["match_evidence"] == [{"kind": "pattern", "name": "route_preference_phrase"}]
    assert projection["candidate"] == {
        "type": "inspect_route_preference",
        "params": {"source": "text_intent"},
    }
    assert projection["agent_route"] == _expected_agent_route(
        "text_intent",
        "inspect_route_preference",
        "inspect_agent_route_preference_projection",
        requires_confirmation=False,
    )
    assert projection["tool_selection"] == {
        "selected_tool": "inspect_agent_route_preference_projection",
        "why_this_tool": "dialog_action_to_agent_tool.inspect_route_preference",
        "availability_checked": False,
    }
...
def test_intent_router_projection_explains_intent_projection_route():
    router = IntentRouter()
    diag = ProjectDiagnosisOut(
        missing_items=[],
        completed_items=["setup", "storyline", "outline", "content"],
    ...
    projection = router.project("检查意图投影：请开始写正文，从第3章开始生成。", "chatting", None, diag).to_dict()

    assert projection["status"] == "matched"
    assert projection["rule_id"] == "intent_projection_intent"
    assert projection["decision"]["rule_id"] == "intent_projection_intent"
    assert projection["decision"]["match_evidence"] == [{"kind": "pattern", "name": "intent_projection_phrase"}]
    assert projection["candidate"] == {
        "type": "inspect_intent_projection",
        "params": {"text": "请开始写正文，从第3章开始生成。"},
    }
    assert projection["agent_route"] == _expected_agent_route(
        "text_intent",
        "inspect_intent_projection",
        "inspect_agent_intent_projection",
        requires_confirmation=False,
    )
    assert projection["tool_selection"] == {
        "selected_tool": "inspect_agent_intent_projection",
        "why_this_tool": "dialog_action_to_agent_tool.inspect_intent_projection",
        "availability_checked": False,
    }
    assert projection["extracted_params"] == {"text": "请开始写正文，从第3章开始生成。"}


def test_intent_router_projection_explains_dialog_control_plane_projection_route():
    router = IntentRouter()
    diag = ProjectDiagnosisOut(
        missing_items=[],
        completed_items=["setup", "storyline", "outline", "content"],
    ...
    projection = router.project("检查 generate_chapter 对话控制面投影", "chatting", None, diag).to_dict()

    assert projection["status"] == "matched"
    assert projection["rule_id"] == "dialog_control_plane_projection_intent"
    assert projection["decision"]["rule_id"] == "dialog_control_plane_projection_intent"
    assert projection["decision"]["match_evidence"] == [
        {"kind": "pattern", "name": "dialog_control_plane_projection_phrase"}
    ]
    assert projection["candidate"] == {
        "type": "inspect_dialog_control_plane",
        "params": {"action_type": "generate_chapter"},
    }
    assert projection["agent_route"] == _expected_agent_route(
        "text_intent",
        "inspect_dialog_control_plane",
        "inspect_agent_dialog_control_plane_projection",
        requires_confirmation=False,
    )
    assert projection["tool_selection"] == {
        "selected_tool": "inspect_agent_dialog_control_plane_projection",
        "why_this_tool": "dialog_action_to_agent_tool.inspect_dialog_control_plane",
        "availability_checked": False,
    }
...
def test_intent_router_projection_explains_agent_health_route():
    router = IntentRouter()
    diag = ProjectDiagnosisOut(
        missing_items=[],
        completed_items=["setup", "storyline", "outline", "content"],
    ...
    projection = router.project("检查第3章 Agent 健康", "chatting", None, diag).to_dict()

    assert projection["status"] == "matched"
    assert projection["rule_id"] == "agent_health_intent"
    assert projection["decision"]["rule_id"] == "agent_health_intent"
    assert projection["decision"]["match_evidence"] == [{"kind": "pattern", "name": "agent_health_phrase"}]
    assert projection["candidate"] == {
        "type": "inspect_agent_health",
        "params": {"chapter_index": 3},
    }
    assert projection["agent_route"] == _expected_agent_route(
        "text_intent",
        "inspect_agent_health",
        "inspect_agent_health_projection",
        requires_confirmation=False,
    )
    assert projection["tool_selection"] == {
        "selected_tool": "inspect_agent_health_projection",
        "why_this_tool": "dialog_action_to_agent_tool.inspect_agent_health",
        "availability_checked": False,
    }
    assert projection["extracted_params"] == {"chapter_index": 3}
        ...
        action_result={
            ...
            "data": {
                ...
                "worker_dispatch": {
                    ...
                    "route_registry": {
                        "status": "passed",
                        "summary": {"routes": 50, "ready_routes": 50, "unrouted_allowed_tools": 0, "issues": 0},
                    },
                },
                "route_decision": {
