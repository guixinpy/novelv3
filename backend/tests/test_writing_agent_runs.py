...
from test_support.writing_agent_run_helpers import (
    approved_generate_chapter_tool as _approved_generate_chapter_tool,
    approved_seed_continuity_anchor_proposals_tool as _approved_seed_continuity_anchor_proposals_tool,
    enqueue_longform_batch_with_approval as _enqueue_longform_batch_with_approval,
    execute_approved_longform_batch_chapter as _execute_approved_longform_batch_chapter,
    preflight_longform_batch_with_approval as _preflight_longform_batch_with_approval,
    prepare_longform_batch_execution_contract as _prepare_longform_batch_execution_contract,
    review_executed_longform_batch_chapter as _review_executed_longform_batch_chapter,
    route_reviewed_longform_batch_after_review as _route_reviewed_longform_batch_after_review,
    seed_longform_project as _seed_longform_project,
    seed_pending_world_proposal as _seed_pending_world_proposal,
)
...
def test_agent_run_recommended_followups_prepare_world_model_analysis_instead_of_writing(
    client,
    db_session,
):
    ...
    db_session.add(
        WritingAgentStep(
            ...
            output={
                ...
                "agent_tool_result": {
                    "recommendations": {
                        "canonical_followups": ["analyze_chapter_world_model"],
                    }
                },
            },
        )
    ...
    assert response.status_code == 200
    assert payload["status"] == "success"
    assert [step["tool_name"] for step in payload["steps"]] == ["prepare_analyze_chapter_world_model_execution"]
    output = payload["steps"][0]["output"]
    assert output["status"] == "approval_required"
    assert output["side_effects"] == {"executed": [], "skipped": ["analyze_chapter_world_model"]}
...
def test_agent_run_routes_passed_longform_batch_review_to_next_batch_preview(client, db_session, monkeypatch):
    ...
    payload = _route_reviewed_longform_batch_after_review(
        client,
        project.id,
        task_id=prepared["task_id"],
        next_batch_size=1,
...
def test_agent_run_post_review_routing_is_idempotent(client, db_session, monkeypatch):
    ...
    first = _route_reviewed_longform_batch_after_review(client, project.id, task_id=prepared["task_id"])
    second = _route_reviewed_longform_batch_after_review(client, project.id, task_id=prepared["task_id"])
