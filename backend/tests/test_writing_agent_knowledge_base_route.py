...
def test_record_agent_knowledge_base_candidate_persists_candidate_and_updates_route(db_session):
    ...
    assert retrieval["items"][0]["source_type"] == "knowledge_base_candidate"
    assert retrieval["items"][0]["title"] == "低细节续写可行"
    assert retrieval["items"][0]["source_ref"].startswith("knowledge_base_candidate:")
...
def test_record_agent_knowledge_base_candidate_removes_muted_candidate_from_retrieval(db_session):
    project = Project(name="Candidate Retrieval Removal", style_config={})
    db_session.add(project)
    db_session.commit()

    created = record_agent_knowledge_base_candidate(
        db_session,
        project.id,
        memory_type="writing_pattern",
        title="章末钩子",
    ...
    assert search_retrieval(db_session, project.id, "章末钩子", source_type="knowledge_base_candidate")["total"] == 1

    muted = record_agent_knowledge_base_candidate(
        db_session,
        project.id,
        memory_type="writing_pattern",
        title="章末钩子",
    ...
    assert created["candidate"]["id"] == muted["candidate"]["id"]
    assert muted["candidate"]["status"] == "muted"
    assert search_retrieval(db_session, project.id, "章末钩子", source_type="knowledge_base_candidate")["total"] == 0
