...
PREPARE_LONGFORM_CHAPTER_BATCH_EXECUTION_REVIEW_VERSION = "phase202.longform_batch_review_prepare.v1"
EXECUTE_LONGFORM_CHAPTER_BATCH_EXECUTION_REVIEW_WITH_APPROVAL_VERSION = (
    "phase202.longform_batch_review_with_approval_execute.v1"
)
APPROVAL_GATE_VERSION = "phase202.longform_batch_execution_review_agent_plan_approval.v1"
TARGET_TYPE = "background_task_post_generation_review"


def prepare_longform_chapter_batch_execution_review(
    db: Session,
    project_id: str,
    *,
    task_id: str | None,
    ...
    preview = review_longform_chapter_batch_execution(
        db,
        project_id,
        task_id=task.id,
        lookback=lookback,
    ...
    if not (preview.get("status") == "blocked" and preview.get("reason") == "review_confirmation_required"):
        return {
            **preview,
            "prepare_gate_version": PREPARE_LONGFORM_CHAPTER_BATCH_EXECUTION_REVIEW_VERSION,
            "target_type": TARGET_TYPE,
            "side_effects": {"executed": [], "skipped": ["background_task_result_post_generation_review"]},
        }

    agent_plan = _direct_review_agent_plan(project_id, task_id=task.id, lookback=lookback)
    ...
    return {
        ...
        "prepare_version": PREPARE_LONGFORM_CHAPTER_BATCH_EXECUTION_REVIEW_VERSION,
        ...
        "side_effects": {"executed": [], "skipped": ["background_task_result_post_generation_review"]},
        "recommended_next_tools": ["execute_longform_chapter_batch_execution_review_with_approval"],
        "trace": {
            "selected_tools": ["prepare_longform_chapter_batch_execution_review"],
            "rejected_tools": [
                {"tool_name": "review_longform_chapter_batch_execution", "reason": "approval_required_before_write"}
            ],
...
def execute_longform_chapter_batch_execution_review_with_approval(
    db: Session,
    project_id: str,
    *,
    task_id: str | None,
    ...
    result = review_longform_chapter_batch_execution(
        db,
        project_id,
        task_id=task.id,
        lookback=lookback,
    ...
    return {
        **result,
        "execute_version": EXECUTE_LONGFORM_CHAPTER_BATCH_EXECUTION_REVIEW_WITH_APPROVAL_VERSION,
        "target_type": TARGET_TYPE,
        "agent_plan_approval_verification": verification,
        "approval_verification_event": build_approval_verification_event(verification),
        "execution_resource_binding": binding_check,
        "evidence": {
            ...
            "legacy_action": "review_longform_chapter_batch_execution",
            "execution_route": "static_adapter",
        },
        "trace": {
            **(result.get("trace") if isinstance(result.get("trace"), dict) else {}),
            "selected_tools": ["execute_longform_chapter_batch_execution_review_with_approval"],
            "approval_gate_version": APPROVAL_GATE_VERSION,
        },
    }


def _direct_review_agent_plan(project_id: str, *, task_id: str, lookback: int | None) -> dict[str, Any]:
    params: dict[str, Any] = {"task_id": task_id}
    if lookback is not None:
        params["lookback"] = int(lookback)
    plan_id = f"direct-longform-batch-execution-review:{task_id}"
    mutation_fingerprint = build_mutation_fingerprint(project_id, "review_longform_chapter_batch_execution", params)
    step = {
        "step_index": 1,
        "step_id": plan_id,
        "tool_name": "review_longform_chapter_batch_execution",
        "approval_executor_tool_name": "execute_longform_chapter_batch_execution_review_with_approval",
        "params": params,
        "mutability": "guarded_write",
        "requires_confirmation": True,
        "mutation_fingerprint": mutation_fingerprint,
        "reason": "审查已生成章节，写入质量、连续性和世界模型分析证据。",
    }
    step.update(
    ...
    return {
        "project_id": project_id,
        "intent_class": "direct_longform_batch_execution_review",
        "trace": {
            "plan_id": plan_id,
            "source_projection_id": None,
            "planner_version": PREPARE_LONGFORM_CHAPTER_BATCH_EXECUTION_REVIEW_VERSION,
        },
        "steps": [step],
    }
...
def _not_found_output(project_id: str, task_id: str | None, *, prepare: bool) -> dict[str, Any]:
    ...
    version = (
        PREPARE_LONGFORM_CHAPTER_BATCH_EXECUTION_REVIEW_VERSION
        if prepare
        else EXECUTE_LONGFORM_CHAPTER_BATCH_EXECUTION_REVIEW_WITH_APPROVAL_VERSION
    )
    return {
        "status": "not_found",
        version_key: version,
        ...
        "trace": {
            "selected_tools": [
                ...
                if prepare
                else "execute_longform_chapter_batch_execution_review_with_approval"
            ],
            "rejected_tools": [{"tool_name": "review_longform_chapter_batch_execution", "reason": "task_not_found"}],
            "approval_gate_version": APPROVAL_GATE_VERSION,
        },
    }
...
def _blocked_output(
    ...
    *,
    ...
    output = {
        ...
        "reason": reason,
        "side_effects": {"executed": [], "skipped": ["background_task_result_post_generation_review"]},
        "recommended_next_tools": ["prepare_longform_chapter_batch_execution_review"],
        "trace": {
            "selected_tools": ["execute_longform_chapter_batch_execution_review_with_approval"],
            "rejected_tools": [{"tool_name": "review_longform_chapter_batch_execution", "reason": reason}],
            "approval_gate_version": APPROVAL_GATE_VERSION,
        },
    }
    if extra:
