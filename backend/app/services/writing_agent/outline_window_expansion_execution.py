...
PREPARE_EXPAND_OUTLINE_WINDOW_EXECUTION_VERSION = "phase230.outline_window_expansion_prepare.v1"
EXECUTE_EXPAND_OUTLINE_WINDOW_WITH_APPROVAL_VERSION = "phase230.outline_window_expansion_with_approval_execute.v1"
APPROVAL_GATE_VERSION = "phase230.outline_window_expansion_agent_plan_approval.v1"


def prepare_expand_outline_window_execution(
    db: Session,
    project_id: str,
    *,
    start_chapter: int,
    ...
    agent_plan = _expand_outline_window_agent_plan(project_id, start_chapter, end_chapter)
    ...
    return {
        ...
        "prepare_version": PREPARE_EXPAND_OUTLINE_WINDOW_EXECUTION_VERSION,
        ...
        "side_effects": {"executed": [], "skipped": ["expand_outline_window"]},
        "recommended_next_tools": ["execute_expand_outline_window_with_approval"],
        "trace": {
            "selected_tools": ["prepare_expand_outline_window_execution"],
            "rejected_tools": [{"tool_name": "expand_outline_window", "reason": "approval_required_before_write"}],
            "approval_gate_version": APPROVAL_GATE_VERSION,
        },
    }


async def execute_expand_outline_window_with_approval(
    db: Session,
    project_id: str,
    *,
    start_chapter: int,
    ...
    if confirm_execute is not True:
        return _blocked_output(project_id, start_chapter, end_chapter, reason="confirmation_required")
    if not approval_contract_hash or not isinstance(approval_contract, dict):
        return _blocked_output(project_id, start_chapter, end_chapter, reason="approval_contract_required")
    if approval_tool_metadata_provider is None:
        return _blocked_output(project_id, start_chapter, end_chapter, reason="agent_plan_tool_metadata_missing")
    ...
    binding_check = verify_resource_binding_target(
        ...
        target_type="outline",
        target_id=_outline_window_target_id(project_id, start_chapter, end_chapter),
    )
    ...
    result = await expand_outline_window_tool(
        db,
        project_id,
        start_chapter=start_chapter,
        end_chapter=end_chapter,
    ...
    return {
        **result,
        "execute_version": EXECUTE_EXPAND_OUTLINE_WINDOW_WITH_APPROVAL_VERSION,
        "project_id": project_id,
        "start_chapter": start_chapter,
        "end_chapter": end_chapter,
        "target_type": "outline",
        ...
        "evidence": {
            **(result.get("evidence") if isinstance(result.get("evidence"), dict) else {}),
            ...
            "legacy_action": "expand_outline_window",
            "execution_route": "static_adapter",
        },
        "side_effects": {"executed": ["expand_outline_window"], "skipped": []},
        "trace": {
            **(result.get("trace") if isinstance(result.get("trace"), dict) else {}),
            "selected_tools": ["execute_expand_outline_window_with_approval"],
            "approval_gate_version": APPROVAL_GATE_VERSION,
        },
    }


def _expand_outline_window_agent_plan(project_id: str, start_chapter: int, end_chapter: int) -> dict[str, Any]:
    params = {"start_chapter": start_chapter, "end_chapter": end_chapter}
    plan_id = f"direct-expand-outline-window:{project_id}:chapters:{start_chapter}-{end_chapter}"
    mutation_fingerprint = build_mutation_fingerprint(project_id, "expand_outline_window", params)
    step = {
        "step_index": 1,
        "step_id": plan_id,
        "tool_name": "expand_outline_window",
    ...
    return {
        "project_id": project_id,
        "intent_class": "direct_expand_outline_window",
        "trace": {
            "plan_id": plan_id,
            "source_projection_id": None,
            "planner_version": PREPARE_EXPAND_OUTLINE_WINDOW_EXECUTION_VERSION,
        },
        "steps": [step],
    }


def _outline_window_target_id(project_id: str, start_chapter: int, end_chapter: int) -> str:
    return f"outline_window:{project_id}:chapters:{start_chapter}-{end_chapter}"
...
def _blocked_output(
    ...
    *,
    ...
    output = {
        ...
        "reason": reason,
        "side_effects": {"executed": [], "skipped": ["expand_outline_window"]},
        "recommended_next_tools": ["prepare_expand_outline_window_execution"],
        "trace": {
            "selected_tools": ["execute_expand_outline_window_with_approval"],
            "rejected_tools": [{"tool_name": "expand_outline_window", "reason": reason}],
            "approval_gate_version": APPROVAL_GATE_VERSION,
        },
    }
    if extra:
