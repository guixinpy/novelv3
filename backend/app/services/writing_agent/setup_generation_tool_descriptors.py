...
_SETUP_PREVIEW_OUTPUT = object_schema(
    {
        "status": {"type": "string"},
...
_SETUP_PREPARE_OUTPUT = object_schema(
    {
        "status": {"type": "string"},
...
SETUP_GENERATION_AGENT_TOOL_DESCRIPTORS: tuple[AgentToolDescriptor, ...] = (
    AgentToolDescriptor(
        name="preview_generate_setup_execution",
        module="writing_agent",
        category="generation",
        description="预览项目基础设定生成的 Agent 写入计划，不执行生成。",
        input_schema=_COMMAND_ARGS_INPUT,
        output_schema=_SETUP_PREVIEW_OUTPUT,
        target_type="setup_generation_preview",
        internal=True,
    ...
    AgentToolDescriptor(
        name="prepare_generate_setup_execution",
        module="writing_agent",
        category="generation",
        description="为项目基础设定生成构建 Agent 计划审批契约，不执行生成。",
        input_schema=_COMMAND_ARGS_INPUT,
        output_schema=_SETUP_PREPARE_OUTPUT,
        target_type="setup_generation_approval",
        internal=True,
        non_blocking_report=True,
        sort_key=22,
        availability_checks=("project_exists",),
    ...
    AgentToolDescriptor(
        name="execute_generate_setup_with_approval",
        module="writing_agent",
        category="generation",
        description="在确认 Agent 计划审批契约后执行项目基础设定生成。",
