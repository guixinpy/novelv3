...
WORLD_MODEL_AGENT_TOOL_DESCRIPTORS: tuple[AgentToolDescriptor, ...] = (
    ...
    AgentToolDescriptor(
        name="prepare_import_setup_world_model_execution",
        module="writing_agent",
        category="athena_world_model",
        description="为设定导入 Athena 世界模型构建 Agent 计划审批契约，不执行导入。",
        input_schema=object_schema(),
        ...
        target_type="world_model_import_approval",
        internal=True,
    ...
    AgentToolDescriptor(
        name="execute_import_setup_world_model_with_approval",
        module="writing_agent",
        category="athena_world_model",
        description="在确认 Agent 计划审批契约后将项目设定导入 Athena 世界模型。",
        input_schema=object_schema(
            {
