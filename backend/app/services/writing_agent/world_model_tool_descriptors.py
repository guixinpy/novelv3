...
WORLD_MODEL_AGENT_TOOL_DESCRIPTORS: tuple[AgentToolDescriptor, ...] = (
    ...
    AgentToolDescriptor(
        name="prepare_analyze_chapter_world_model_execution",
        module="writing_agent",
        category="athena_world_model",
        description="为章节世界模型分析构建 Agent 计划审批契约，不写入提案。",
        input_schema=_CHAPTER_PARAMS,
        output_schema=object_schema(
            {
                "status": {"type": "string"},
        ...
        target_type="world_model_analysis_approval",
        internal=True,
        non_blocking_report=True,
        sort_key=71,
        availability_checks=("generated_chapter_exists", "world_model_profile_exists"),
    ...
    AgentToolDescriptor(
        name="execute_analyze_chapter_world_model_with_approval",
        module="writing_agent",
        category="athena_world_model",
        description="在确认 Agent 计划审批契约后分析章节并写入 Athena 世界模型提案。",
        input_schema=object_schema(
            {
    ...
    AgentToolDescriptor(
        name="prepare_seed_continuity_anchor_proposals_execution",
        module="writing_agent",
        category="maintenance",
        description="为关键连续性锚点提案补齐构建 Agent 计划审批契约，不写入提案。",
        input_schema=object_schema(),
        output_schema=object_schema(
            {
                "status": {"type": "string"},
        ...
        target_type="world_model_continuity_anchor_seed_approval",
        internal=True,
        non_blocking_report=True,
        sort_key=211,
        availability_checks=("world_model_profile_exists",),
    ...
    AgentToolDescriptor(
        name="execute_seed_continuity_anchor_proposals_with_approval",
        module="writing_agent",
        category="maintenance",
        description="在确认 Agent 计划审批契约后为关键连续性锚点生成世界模型提案。",
        input_schema=object_schema(
            {
                "confirm_execute": {"type": "boolean"},
                "approval_contract_hash": {"type": "string"},
